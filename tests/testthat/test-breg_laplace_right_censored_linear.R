data("penguins")
test_data = na.omit(penguins) %>% dplyr::rename(y = body_mass, x1 = flipper_len, x2 = bill_len) |> dplyr::mutate(y = ifelse(y > 5000, 5000, y))
...
test_that("similar CI's", {
  expect_equal(posterior_interval(fit, prob = 0.95), confint(compare, level = 0.95)[1:3,], tolerance = 1e-1)
})
