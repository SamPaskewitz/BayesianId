set.seed(1212)
test_data = data.frame(x1 = rnorm(100), f = rep(c("a", "b"), times = 50) |> as.factor()) |>
  dplyr::mutate(mu = 1 + 1.5*x1 + 0.3*(f == "a")) |>
  dplyr::mutate(y = rbinom(100, size = 10, prob = plogis(mu)))

fit = breg_laplace(y ~ x1 + f, data = test_data, family = "binomial_logistic", n_trials = 10, prior_scale = 100) # large prior scale -> results should be similar to MLE
compare = glm(cbind(y, 10 - y) ~ x1 + f, data = prepare_data(parse_formula(y ~ x1 + f), test_data), family = binomial())
...
test_that("predict (on the response scale) gives similar results", {
  expect_equal(predict(fit) |> unname(),
               10*(predict(compare, type = "response") |> unname()),
               tolerance = 1e-2)
})
