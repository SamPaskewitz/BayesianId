...
test_that("lhs_works", {
  ...
  expect_match(parse_formula(fmla4)$lhs, "sqrt(y)", fixed = TRUE)
})
...
test_that("fixed_works", {
  ...
  expect_setequal(parse_formula(fmla4)$fixed, c("x1", "x2"))
})
...
test_that("fixed_main_works", {
  ...
  expect_setequal(parse_formula(fmla4)$fixed_main, c("x1", "x2"))
})
