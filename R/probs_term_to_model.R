...
probs_term_to_model = function(main_probs, intr_condprobs, model_list){
  ...
  model_probs = rep(1.0, times = model_list$n_models)
  names(model_probs) = model_list$model_names

  # adjust model probabilities for main effects
  for(i in 1:model_list$n_main){ # main effect probs are not conditional
    main = model_list$main_names[i]
    has_main = model_list$included_table[,main]
  ...
  if(model_list$n_intr > 0){
    for(i in 1:model_list$n_intr){
      intr = model_list$intr_names[i]
      has_intr = model_list$included_table[,intr]
      could_include = lapply(model_list$included, function(x){has_needed(x, intr)}) |> unlist()
